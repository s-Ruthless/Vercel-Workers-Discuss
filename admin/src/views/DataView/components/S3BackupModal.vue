        ...
        <div v-else class="backup-list">
          <div v-for="item in backups" :key="item.key" class="backup-item">
            ...
            <div class="backup-actions">
              <button class="backup-btn download" @click="handleDownload(item.key)" :title="t('data.sections.s3.download')">
                <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
              </button>
              <button class="backup-btn delete" @click="handleDelete(item.key)" :disabled="deletingKey === item.key" :title="t('data.sections.s3.delete')">
                <svg v-if="deletingKey !== item.key" viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
...
import { fetchS3BackupList, deleteS3Backup, type S3BackupItem } from "../../../api/admin";
...
const handleClose = () => {
  if (loading.value || deletingKey.value) return;
  emit("close");
};
...
const handleDownload = async (key: string) => {
  try {
    ...
    const url = `/api/admin/backup/s3/download?key=${encodeURIComponent(key)}`;
...
const handleDelete = async (key: string) => {
  if (!confirm(t("data.sections.s3.confirmDelete", { file: key }))) return;
