  ...
  <div class="page">
    ...
    <div v-else>
      <div class="settings-tabs">
        <button type="button" class="settings-tab" :class="{ 'settings-tab-active': activeTab === 'comment' }" @click="activeTab = 'comment'">{{ t("settings.tabs.comment") }}</button>
        <button type="button" class="settings-tab" :class="{ 'settings-tab-active': activeTab === 'feature' }" @click="activeTab = 'feature'">{{ t("settings.tabs.feature") }}</button>
        <button type="button" class="settings-tab" :class="{ 'settings-tab-active': activeTab === 'emailNotify' }" @click="activeTab = 'emailNotify'">{{ t("settings.tabs.emailNotify") }}</button>
        <button type="button" class="settings-tab" :class="{ 'settings-tab-active': activeTab === 'telegramNotify' }" @click="activeTab = 'telegramNotify'">{{ t("settings.tabs.telegramNotify") }}</button>
        <button type="button" class="settings-tab" :class="{ 'settings-tab-active': activeTab === 'display' }" @click="activeTab = 'display'">{{ t("settings.tabs.display") }}</button>
      </div>
      <transition name="tab-fade" mode="out-in">
        <div :key="activeTab" class="settings-content">
          ...
          <template v-if="activeTab === 'comment'">
            <div class="card">
              <div class="card-header"><div class="card-title">{{ t("settings.comment.title") }}</div></div>
              <div class="card-body">
                <div class="form-item">
                ...
                <div class="form-item">
                  ...
                  <label class="switch"><input v-model="commentAdminEnabled" type="checkbox" /><span class="slider" /></label>
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.comment.avatarPrefix") }}</label>
                ...
                <div class="form-item">
                  ...
                  <label class="switch"><input v-model="requireReview" type="checkbox" /><span class="slider" /></label>
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.comment.adminKey") }}</label>
                  <input v-model="commentAdminKey" class="form-input" :placeholder="t('settings.comment.adminKeyPlaceholder')" autocomplete="new-password" />
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.comment.allowedDomains") }}</label>
                  <TagInput v-model="allowedDomainTags" />
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.comment.blockedIps") }}</label>
                  <TagInput v-model="blockedIpTags" />
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.comment.blockedEmails") }}</label>
          ...
          <template v-else-if="activeTab === 'feature'">
            <div class="card feature">
              <div class="card-header"><div class="card-title">{{ t("settings.feature.title") }}</div></div>
              <div class="card-body">
                <div class="form-item bg">
                  <div class="form-item-flex">
                    <label class="form-label">{{ t("settings.feature.articleLike") }}</label>
                    <label class="switch"><input v-model="enableArticleLike" type="checkbox" /><span class="slider" /></label>
                  </div>
                  <div class="form-hint">{{ t("settings.feature.articleLikeHint") }}</div>
                </div>
                <div class="form-item bg">
                  <div class="form-item-flex">
                    <label class="form-label">{{ t("settings.feature.commentLike") }}</label>
                    <label class="switch"><input v-model="enableCommentLike" type="checkbox" /><span class="slider" /></label>
                  </div>
                  <div class="form-hint">{{ t("settings.feature.commentLikeHint") }}</div>
                </div>
                <div class="form-item bg">
                  <div class="form-item-flex">
                    <label class="form-label">{{ t("settings.feature.imageLightbox") }}</label>
                    <label class="switch"><input v-model="enableImageLightbox" type="checkbox" /><span class="slider" /></label>
                  </div>
                  <div class="form-hint">{{ t("settings.feature.imageLightboxHint") }}</div>
                </div>
                <div class="form-item bg">
                  <div class="form-item-flex">
                    <label class="form-label">{{ t("settings.feature.emoji") }}</label>
                    <label class="switch"><input v-model="enableEmoji" type="checkbox" /><span class="slider" /></label>
                  </div>
...
<button class="card-button" :disabled="savingFeature" @click="saveFeature">
          ...
          <template v-else-if="activeTab === 'display'">
            <div class="card">
              <div class="card-header"><div class="card-title">{{ t("settings.display.title") }}</div></div>
              <div class="card-body">
                <div class="form-item">
          ...
          <template v-else-if="activeTab === 'emailNotify'">
            <div class="card">
              <div class="card-header"><div class="card-title">{{ t("settings.emailNotify.title") }}</div></div>
              <div class="card-body">
                <div class="form-item">
                  <label class="form-label">{{ t("settings.emailNotify.enable") }}</label>
                  <label class="switch"><input v-model="emailGlobalEnabled" type="checkbox" /><span class="slider" /></label>
                </div>
                ...
                <div v-if="smtpProvider === 'custom'">
                  ...
                  <div class="form-item">
                    ...
                    <label class="switch"><input v-model="smtpSecure" type="checkbox" /><span class="slider" /></label>
                  </div>
                </div>
                <div class="form-item">
                ...
                <div class="form-item">
                  ...
                  <input v-model="smtpPass" class="form-input" type="text" placeholder="QQ邮箱请使用授权码" />
                ...
                <div class="form-item">
                  ...
                  <div class="form-hint">{{ t("settings.emailNotify.adminTemplateHint") }}</div>
                  <textarea v-model="templateAdmin" class="form-input" rows="10" placeholder="留空则使用默认模板"></textarea>
                </div>
                <div class="form-item">
                ...
                <div class="card-actions" style="justify-content: space-between">
                  ...
                  <button class="card-button secondary" style="margin-left: auto" @click="resetTemplatesToDefault">{{ t("settings.emailNotify.resetBtn") }}</button>
          ...
          <template v-else-if="activeTab === 'telegramNotify'">
            <div class="card">
              <div class="card-header"><div class="card-title">{{ t("settings.telegramNotify.title") }}</div></div>
              <div class="card-body">
                <div class="form-item">
                  <label class="form-label">{{ t("settings.telegramNotify.enable") }}</label>
                  <label class="switch"><input v-model="telegramNotifyEnabled" type="checkbox" /><span class="slider" /></label>
                </div>
                <div class="form-item">
                  <label class="form-label">{{ t("settings.telegramNotify.botToken") }}</label>
                  <input v-model="telegramBotToken" class="form-input" type="password" placeholder="123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11" autocomplete="new-password" />
                ...
                <div class="form-item">
                  ...
                  <input v-model="telegramChatId" class="form-input" type="text" placeholder="123456789" />
                  <div class="form-hint" v-html="t('settings.telegramNotify.chatIdHint')"></div>
                </div>
                <div class="card-actions" style="justify-content: space-between">
                  <button class="card-button secondary" :disabled="settingUpWebhook" @click="doSetupWebhook">
                  ...
                  <button class="card-button secondary" :disabled="testingTelegram" @click="testTelegram" style="margin-right: auto">
                    <span v-if="testingTelegram">{{ t("settings.telegramNotify.testingBtn") }}</span>
                    <span v-else>{{ t("settings.telegramNotify.testBtn") }}</span>
                  </button>
                  <button class="card-button" :disabled="savingTelegram" @click="saveTelegram">
...
import {
  fetchCommentSettings, saveCommentSettings,
  fetchEmailNotifySettings, saveEmailNotifySettings, sendTestEmail,
  fetchFeatureSettings, saveFeatureSettings,
  fetchAdminDisplaySettings, saveAdminDisplaySettings,
  fetchTelegramSettings, saveTelegramSettings, setupTelegramWebhook, sendTelegramTestMessage,
} from "../../api/admin";
...
const adminLayoutTitle = ref("VWD 评论系统");
...
const activeTab = ref<TabKey>(validTabs.includes(route.query.tab as TabKey) ? (route.query.tab as TabKey) : "comment");

watch(activeTab, (newTab) => { router.replace({ query: { ...route.query, tab: newTab } }); });
watch(() => route.query.tab, (newTab) => {
  if (newTab && validTabs.includes(newTab as TabKey)) activeTab.value = newTab as TabKey;
});

const savingEmail = ref(false);
const testingEmail = ref(false);
const emailGlobalEnabled = ref(false);
  ...
  <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;color:#111827;">
    ...
    <div style="padding:24px 28px;">
      <p style="margin:0 0 16px 0;font-size:14px;color:#4b5563;">
        <span style="font-weight:600;">\${replyAuthor}</span> 回复了你在 <span style="font-weight:600;">《\${postTitle}》</span> 中的评论：
      </p>
  ...
  <div style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;color:#111827;">
    ...
    <div style="padding:24px 28px;">
      <p style="margin:0 0 10px 0;font-size:14px;color:#374151;">
        <span style="font-weight:600;">\${commentAuthor}</span> 在文章 <span style="font-weight:600;">《\${postTitle}》</span> 下发表了新评论：
      </p>
...
function onProviderChange() {
  ...
  else if (smtpProvider.value === "163") { smtpHost.value = "smtp.163.com"; smtpPort.value = 465; smtpSecure.value = true; }
}
...
async function load() {
  ...
  try {
    const [commentRes, emailNotifyRes, featureRes, telegramRes, displayRes] = await Promise.all([
      fetchCommentSettings(), fetchEmailNotifySettings(), fetchFeatureSettings(), fetchTelegramSettings(), fetchAdminDisplaySettings(),
    ]);
    ...
    allowedDomainTags.value = (Array.isArray(commentRes.allowedDomains) ? commentRes.allowedDomains : []).map((d: string) => String(d).trim()).filter(Boolean);
    blockedIpTags.value = Array.isArray(commentRes.blockedIps) ? commentRes.blockedIps : [];
    blockedEmailTags.value = Array.isArray(commentRes.blockedEmails) ? commentRes.blockedEmails : [];
...
emojiPathsText.value = Array.isArray(featureRes.emojiPaths) ? featureRes.emojiPaths.join("\n") : "";
    ...
    adminLayoutTitle.value = displayRes.layoutTitle || "VWD 评论系统";
    ...
    if (emailNotifyRes.smtp) {
      ...
      if (emailNotifyRes.smtp.host === "smtp.qq.com") smtpProvider.value = "qq";
      else if (emailNotifyRes.smtp.host === "smtp.163.com") smtpProvider.value = "163";
...
async function saveEmail() {
  savingEmail.value = true; message.value = "";
  try {
...
async function testEmail() {
  if (!commentAdminEmail.value) { message.value = "请先设置管理员邮箱"; messageType.value = "error"; return; }
  if (!smtpUser.value || !smtpPass.value) { message.value = "请先填写 SMTP 账号和密码"; messageType.value = "error"; return; }
  testingEmail.value = true; message.value = "";
  try {
    const res = await sendTestEmail({ toEmail: commentAdminEmail.value, smtp: { host: smtpHost.value, port: smtpPort.value, user: smtpUser.value, pass: smtpPass.value, secure: smtpSecure.value } });
    ...
    if (errorMsg.includes("535") && (errorMsg.includes("Login fail") || errorMsg.includes("authentication failed"))) {
      errorMsg = '验证失败 (535)：请检查 1. QQ 邮箱是否已开启 POP3/SMTP 服务；2. 密码栏是否填写了"授权码"（非 QQ 密码）。';
    }
    message.value = errorMsg; messageType.value = "error";
  } finally { testingEmail.value = false; }
}

async function saveComment() {
  savingComment.value = true; message.value = "";
  try {
    const res = await saveCommentSettings({
      adminEmail: commentAdminEmail.value, adminBadge: commentAdminBadge.value, avatarPrefix: avatarPrefix.value,
      adminEnabled: commentAdminEnabled.value, allowedDomains: allowedDomainTags.value,
      adminKey: commentAdminKey.value || undefined, requireReview: requireReview.value,
      blockedIps: blockedIpTags.value, blockedEmails: blockedEmailTags.value,
    });
...
async function saveTelegram() {
  savingTelegram.value = true; message.value = "";
  try {
    const res = await saveTelegramSettings({ botToken: telegramBotToken.value, chatId: telegramChatId.value, notifyEnabled: telegramNotifyEnabled.value });
    showToast(res.message || "保存成功", "success");
  } catch (e: any) { message.value = e.message || "保存失败"; messageType.value = "error"; }
...
async function testTelegram() {
  if (!telegramBotToken.value || !telegramChatId.value) { showToast("请先填写 Bot Token 和 Chat ID 并保存", "error"); return; }
  testingTelegram.value = true;
  try {
    await saveTelegramSettings({ botToken: telegramBotToken.value, chatId: telegramChatId.value, notifyEnabled: telegramNotifyEnabled.value });
...
async function doSetupWebhook() {
  if (!telegramBotToken.value) { showToast("请先填写 Bot Token 并保存", "error"); return; }
  settingUpWebhook.value = true;
  try {
    await saveTelegramSettings({ botToken: telegramBotToken.value, chatId: telegramChatId.value, notifyEnabled: telegramNotifyEnabled.value });
    const res = await setupTelegramWebhook();
    showToast(res.message || "Webhook 设置成功", "success");
  } catch (e: any) { showToast(e.message || "设置失败", "error"); }
  finally { settingUpWebhook.value = false; }
}
