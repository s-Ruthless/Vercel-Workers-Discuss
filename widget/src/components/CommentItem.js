...
export class CommentItem extends Component {
  ...
  render() {
    const { comment, isReply, adminBadge } = this.props;
    ...
    const root = this.createElement('div', {
      className: `vwd-comment-item ${isReply ? 'vwd-comment-reply' : ''}`,
      children: [
        // 头像
        this.createElement('div', {
          className: 'vwd-comment-avatar',
          children: [
        ...
        // 主体内容
        this.createElement('div', {
          className: 'vwd-comment-body',
          children: [
            // 头部
            this.createElement('div', {
              className: 'vwd-comment-header',
              children: [
                // 作者信息
                this.createElement('div', {
                  className: 'vwd-comment-author',
                  children: [
                    comment.url
                      ? this.createElement('span', {
                          className: 'vwd-author-name',
                          children: [
                      ...
                      : this.createTextElement('span', comment.name, 'vwd-author-name'),
                    ...(isAdmin
                      ? [
                          adminBadge
                            ? this.createTextElement('span', `${adminBadge}`, 'vwd-admin-badge')
                            : this.createElement('span', {
                                className: 'vwd-admin-badge vwd-admin-badge-icon',
                                attributes: { title: '网站管理员' },
                                html: '<svg viewBox="0 0 22 22" aria-label="网站管理员" role="img" class="vwd-admin-icon" style="width:15px;height:15px;fill:currentColor;vertical-align:-0.15em;"><g><path d="M20.396 11c-.018-.646-.215-1.275-.57-1.816-.354-.54-.852-.972-1.438-1.246.223-.607.27-1.264.14-1.897-.131-.634-.437-1.218-.882-1.687-.47-.445-1.053-.75-1.687-.882-.633-.13-1.29-.083-1.897.14-.273-.587-.704-1.086-1.245-1.44S11.647 1.62 11 1.604c-.646.017-1.273.213-1.813.568s-.969.854-1.24 1.44c-.608-.223-1.267-.272-1.902-.14-.635.13-1.22.436-1.69.882-.445.47-.749 1.055-.878 1.688-.13.633-.08 1.29.144 1.896-.587.274-1.087.705-1.443 1.245-.356.54-.555 1.17-.574 1.817.02.647.218 1.276.574 1.817.356.54.856.972 1.443 1.245-.224.606-.274 1.263-.144 1.896.13.634.433 1.218.877 1.688.47.443 1.054.747 1.687.878.633.132 1.29.084 1.897-.136.274.586.705 1.084 1.246 1.439.54.354 1.17.551 1.816.569.647-.016 1.276-.213 1.817-.567s.972-.854 1.245-1.44c.604.239 1.266.296 1.903.164.636-.132 1.22-.447 1.68-.907.46-.46.776-1.044.908-1.681s.075-1.299-.165-1.903c.586-.274 1.084-.705 1.439-1.246.354-.54.551-1.17.569-1.816zM9.662 14.85l-3.429-3.428 1.293-1.302 2.072 2.072 4.4-4.794 1.347 1.246z"></path></g></svg>',
                              }),
                        ]
                      : []),
                    ...(isPinned ? [this.createTextElement('span', '置顶', 'vwd-pin-badge')] : []),
                    ...(comment.replyToAuthor
                      ? [
                          this.createTextElement('span', ` ${this.t('reply')} `, 'vwd-reply-to-separator'),
                          this.createTextElement('span', comment.replyToAuthor, 'vwd-reply-to-author'),
                        ]
                      : []),
                  ],
                }),
                ...
                // 操作区域
                this.createElement('div', {
                  className: 'vwd-comment-actions',
                  children: [
                    this.createElement('span', {
                      className: 'vwd-action-btn',
                      attributes: {
                    ...
                    ...(this.props.enableCommentLike !== false
                      ? [
                          this.createElement('div', {
                            className: 'vwd-comment-like',
                            children: [
                              this.createElement('button', {
                                className: `vwd-comment-like-button${this.hasLiked(comment.id) ? ' vwd-comment-like-button-liked' : ''}`,
                                attributes: {
                                  type: 'button',
                                ...
                                children: [
                                  this.createElement('span', {
                                    className: 'vwd-comment-like-icon-wrapper',
                                    children: [
                                      this.createElement('svg', {
                                        className: 'vwd-comment-like-icon',
                                        attributes: {
                                          viewBox: '0 0 24 24',
                                          'aria-hidden': 'true',
                                  ...
                                  ...(() => {
                                    const likeCount =
                                      typeof comment.likes === 'number' && Number.isFinite(comment.likes) && comment.likes >= 0
                                        ...
                                        : 0;
                                    return likeCount >= 1
                                      ? [this.createTextElement('span', String(likeCount), 'vwd-comment-like-count')]
                                      : [];
                                  })(),
                                ],
                              }),
                      ...
                      : []),
                    this.createTextElement('span', formatRelativeTime(comment.created, this.t), 'vwd-comment-time'),
                  ],
                }),
              ],
            }),
            ...
            // 评论内容
            this.createElement('div', {
              className: 'vwd-comment-content',
            }),

            // 回复编辑器容器
            this.createElement('div', {
              className: 'vwd-reply-editor-container',
            }),
    ...
    // 创建回复编辑器
    if (isReplying) {
      const replyContainer = root.querySelector('.vwd-reply-editor-container');
      if (replyContainer) {
    ...
    if (comment.replies && comment.replies.length > 0) {
      const repliesContainer = root.querySelector('.vwd-replies');
      if (repliesContainer) {
  ...
  updateProps(prevProps) {
    ...
    if (isReplying !== wasReplying) {
      const replyContainer = this.elements.root?.querySelector(':scope > .vwd-comment-body > .vwd-reply-editor-container');
  ...
  handleLikeComment() {
    if (!this.props.onLikeComment) return;
    ...
    if (lastClick && now - lastClick < 1000) return;
  ...
  getUserId() {
    if (CommentItem._userId) return CommentItem._userId;

    const STORAGE_KEY = 'vwd_comment_user_id';
  ...
  getLikedComments() {
    ...
    const key = `vwd_comment_liked_${userId}`;
