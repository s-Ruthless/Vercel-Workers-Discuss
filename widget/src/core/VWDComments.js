...
export class VWDComments {
  ...
  mount() {
    ...
    if (this.hostElement) {
      ...
      this.mountPoint.className = 'vwd-comments-container';
  ...
  _render() {
    ...
    // 错误提示
    const existingError = this.mountPoint.querySelector('.vwd-error-inline');
    if (state.error) {
      if (!existingError) {
        const errorEl = document.createElement('div');
        errorEl.className = 'vwd-error-inline';
        errorEl.innerHTML = `
          <span>${state.error}</span>
          <button type="button" class="vwd-error-close" data-action="clear-error">✕</button>
        `;
    ...
    // 成功提示
    const existingSuccess = this.mountPoint.querySelector('.vwd-success-inline');
    if (state.successMessage) {
      if (!existingSuccess) {
        const successEl = document.createElement('div');
        successEl.className = 'vwd-success-inline';
        successEl.innerHTML = `
          <span>${state.successMessage}</span>
          <button type="button" class="vwd-error-close" data-action="clear-success">✕</button>
        `;
        ...
        if (span) span.textContent = state.successMessage;
      }
    ...
    // 头部点赞区域
    let header = this.mountPoint.querySelector('.vwd-comments-header');
    if (!header) {
      header = document.createElement('div');
      header.className = 'vwd-comments-header';
      const showArticleLike = this.config.enableArticleLike !== false;
      header.innerHTML = `
        <div class="vwd-like" ${showArticleLike ? '' : 'style="display: none;"'}>
          <button type="button" class="vwd-like-button" data-liked="false">
            <span class="vwd-like-icon-wrapper">
              <svg class="vwd-like-icon" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div>${this.t('likes', { count: '<span class="vwd-like-count">0</span>' })}</div>
          </button>
        </div>
      `;
    ...
    // 评论数统计
    let countHeading = this.mountPoint.querySelector('.vwd-comments-count');
    if (!countHeading) {
      countHeading = document.createElement('h3');
      countHeading.className = 'vwd-comments-count';
      countHeading.innerHTML = `${this.t('totalComments', { count: '<span class="vwd-comments-count-number">0</span>' })}`;
    ...
    const countEl = this.mountPoint.querySelector('.vwd-comments-count-number');
    if (countEl) {
  ...
  _onStateChange(state, prevState) {
    ...
    if (typeof state.likeCount === 'number' || typeof state.liked === 'boolean') {
      ...
      const header = this.mountPoint?.querySelector('.vwd-comments-header');
      if (header) {
    ...
    const pageChanged = prevState && prevState.pagination && state && state.pagination && state.pagination.page !== prevState.pagination.page;
    if (pageChanged) {
  ...
  _initLikeButton(header) {
    ...
    if (!this._likeButtonEl) {
      this._likeButtonEl = header.querySelector('.vwd-like-button');
      if (this._likeButtonEl) {
    ...
    if (!this._likeCountEl) {
      this._likeCountEl = header.querySelector('.vwd-like-count');
    }
    this._updateLikeButton();
  }
  ...
  _updateLikeButton(animate = false) {
    ...
    if (animate && this._likeButtonEl) {
      this._likeButtonEl.classList.remove('vwd-like-animate');
      void this._likeButtonEl.offsetWidth;
      this._likeButtonEl.classList.add('vwd-like-animate');
    }
  }

  _handleLikeClick() {
    if (!this.api || typeof this.api.likePage !== 'function') return;
    if (this.likeState.loading) return;
